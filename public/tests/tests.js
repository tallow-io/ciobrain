import {By,Key,Builder, Capabilities, Capability, promise} from "selenium-webdriver";
import "chromedriver";
import "geckodriver";
...
async function displayAssetDetails(browser) {
        ...
        const assetCategoryOptions = await driver.findElements(By.className("assetCategoryOption"));

        // Parse through list of asset category options
        for(let category of assetCategoryOptions) {
                await category.click();
                // Retireve asset options list
                const assetOptions = await driver.findElements(By.className("assetOption"));
                for(let option of assetOptions) {
                        ...
                        if(assetName != "") {
                                ...
                                if(await driver.findElement(By.id("assetName")).getText() != assetName) {
                                        throw "Selected asset (" + assetName + ") does not match asset details in " + (browser);
                                }
                        }
                }
        }
...
async function verifyGraphNodes(browser) {
        ...
        for(let category of assetCategoryOptions) {
                ...
                if(categoryName == "Application")
                        ...
                        assets = await asset.getAllDataAssets();
                ...
                for(let option of assetOptions) {
                        ...
                        if (assetName != "") {
                                ...
                                if(selectedAsset) {
                                        let expectedNumberOfNodes = getAssetConnections(selectedAsset) + 1;
                                        let numberOfNodesDisplayed = (await driver.findElements(By.css("circle"))).length;
                                        if(expectedNumberOfNodes != numberOfNodesDisplayed)
                                                throw "Selected asset (" + assetName + ") is not showing correct number of nodes"
                                }
                        }
                }
        }
...
async function verifyGraphExpand(browser) {
        ...
        for(let category of assetCategoryOptions) {
                ...
                for(let option of assetOptions) {
                        ...
                        if(assetName != "") {
                                await option.click();
                                let expandButtons = await driver.findElements(By.css("polygon[points]"));
                                while(expandButtons != "") {
                                        await expandButtons[0].click();
                                        expandButtons = await driver.findElements(By.css("polygon[points]"));
                                }
                        }      
                }
        }
...
async function displayNodeColors(browser){
        ...
        await driver.findElement(By.css("#assetMenu > div:nth-child(2) > div.assetCategoryDropdown > div:nth-child(1)")).click();
