...
export default class AssetMenu extends Component {
    constructor(props) {
        super(props)
        this.state = {
            selectedCategory: null, selectedAssetKey: null
        }
    }
    ...
    render() {
        return (<div id="assetMenu" className="card">
            <div id="assetMenuHeader">
                <div>Assets</div>
                <AssetImport/>
            </div>
            {Object.values(AssetCategoryEnum).map(category => (<AssetCategoryOption
                key={category.name}
                category={category.name}
                selected={this.state.selectedCategory === category.name}
                color={category.color}
                backgroundColor={category.name === this.state.selectedCategory ? category.backgroundColor : "white"}
